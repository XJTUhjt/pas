...
from rl.pas_rnn_model_final import Label_VAE
    ...
    with torch.no_grad():
        for vector, label_grid, sensor_grid,mask_grid, id_grid in loader:
            ...
            *_grid : (T*N, 120, 120)
            ...
            mu_l, logvar_l, z_l, decoded_l = model(label_grid,mask_grid)     #这里还不知道怎么改
...
def Label_vae_train(beta, logging, train_loader, validation_loader, model, ckpt_path, num_epochs, learning_rate = 0.001):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for label_grid, sensor_grid, mask_grid in loop: 
            ...
            mu_l, logvar_l, z_l, decoded_l = model(label_grid,mask_grid)   #这里也还没有改
            #**********这里原来是label_grid********************
            recon_loss, k_loss = VAE_loss(label_grid, decoded_l, mu_l, logvar_l)
            loss = recon_loss + k_loss * beta
...
if __name__ == "__main__":
    ...
    output_path = 'LabelVAE_CircleFOV317'
    ...
    if encoder_type == 'vae':
        ...
        label_vae = nn.DataParallel(label_vae, device_ids=[0,2,3]) # Use DataParallel to pac the model
