...
def main():
	...
	if config.robot.policy == 'pas_rnn' or config.robot.policy == 'srnn' or Config.robot.policy == 'vector_net':
	...
	for j in range(num_updates): 
		...
		for step in range(algo_args.num_steps):
			...
			# If done then clean the history of observations.如果一个episode结束了要清除所有mask
		...
		with torch.no_grad():
			...
			next_value = actor_critic.get_value(obs, rollouts_hidden_s, masks).detach()
		...
		if (j % algo_args.save_interval == 0 or j == num_updates - 1) and j!=0:
