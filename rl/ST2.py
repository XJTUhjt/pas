...
class Attention(nn.Module):
    ...
    def transpose_multihead(self, x):
        new_shape = list(x.shape[:-1]) + [self.num_heads, self.attn_head_size]   #[3,5,8,16]
        x = x.reshape(new_shape)
        x = x.permute([0, 2, 1, 3])
        return x

    def forward(self, x):
        size = x.shape #(3,5,128)

        qkv = self.qkv(x).chunk(3, -1) #qkv是列表，每个是(3,5,128) 将输入 x 经过线性变换得到 Q、K 和 V，并将它们分别存储在变量 qkv 中
        q, k, v = map(self.transpose_multihead, qkv)            #  (3,8,5,16)
        k = torch.mean(q, 2, keepdim=True)                      #  (3,8,1,16)
        k = k.expand((size[0], 8, size[1], -1))                 #  (3,8,5,16)
        k = k.permute([0, 1, 3, 2])                             #  (3,8,16,5)
        attn = torch.matmul(q, k)                               #  (3,8,5,5)
        ...
        z = torch.matmul(attn, v)                               #(3,8,5,16)
        z = z.permute([0, 2, 1, 3])                             #(3,5,8,16)

        new_shape = list(z.shape[:-2]) + [self.all_head_size]
        z = z.reshape(new_shape)                                #(3,5,128)
        z = self.out(z)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        h = x  #(3,5,128)
        x = self.attn_norm(x) #(3,5,128)
        x = self.attn(x)        #(3,5,128)
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        for layer in self.layers:
            x = layer(x)
        out = self.encoder_norm(x)  #(3,5,128)
        return out
...
class Spatial_Temporal_Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # spatial_Transformer
        x = x.reshape(-1, h, w)  #[3,5,13]
        spatial_state = self.Embedding(x) #(3,5,128)
        spatial_state = self.Spatial_encoder(spatial_state).reshape(b, t, h, -1) #(1,3,5,128)
        # temporal_Transformer
        temporal_state = spatial_state.permute([0, 2, 1, 3]).reshape(b*h, t, -1) #(5, 3, 128)
        temporal_state = self.temporal_encoder(temporal_state) #(5,3,128)
        temporal_state = temporal_state.reshape(b, h, t, -1).permute([0, 2, 1, 3])#(1,3,5,128)
        state = temporal_state.reshape(b, t*h, -1) #(1,15,128)

        state = state.permute([0, 2, 1]) #(1,128,15)
        state = self.avgpool(state).flatten(1) #(1,128)
        state = torch.cat((state, robot_state), 1) #(1,134)
