...
# from trans import OcclusionQueries
from crowd_nav.configs.config import Config
# from crowd_sim.envs.generateMaskGrid import generateMaskGrid
...
def make_sequence(data_path, phase, sequence=1):    
    ...
    mask_grid_files = sorted(glob.glob(data_path+'*mask_grid.npy')) #new 
    if phase !='train':
    ...
    sensor_grid = []
    id_grid = []
    mask_grid = []  # Added mask grid
    for i in range(len(label_grid_files)):
        if phase != 'train':
            v_f, lg_f, sg_f, mg_f, id_f = vector_files[i], label_grid_files[i], sensor_grid_files[i], mask_grid_files[i], id_grid_files[i] 
        else:
            lg_f, sg_f, mg_f = label_grid_files[i], sensor_grid_files[i], mask_grid_files[i]
        
        epi_label_grid = np.load(lg_f, mmap_mode='r')
        epi_sensor_grid = np.load(sg_f, mmap_mode='r')
        epi_mask_grid = np.load(mg_f, mmap_mode='r')
        if phase != 'train':
        ...
        for k in range(len(epi_label_grid)):   
            if phase == 'train':  
                lg, sg, mg = epi_label_grid[k], epi_sensor_grid[k], epi_mask_grid[k]
            else:
                v, lg,sg, mg,ig = epi_vector[k], epi_label_grid[k], epi_sensor_grid[k], epi_mask_grid[k], epi_id_grid[k]
            if timestamp == 0.:
                lg = lg # (100, 100)
                sg = sg
                mg = mg
                ...
                maskG = deque(maxlen=sequence)
                if phase != 'train':
                    vec = deque(maxlen=sequence)
                    idG = deque(maxlen=1)

                sensorG.extend(np.vstack([sg for i in range(sequence)]))
                maskG.extend(np.vstack([mg for i in range(sequence)]))
            ...
            mask_grid.append(np.array(maskG, dtype=np.float32).copy())
            if phase != 'train':
    ...
    if phase == 'train':
        return label_grid, sensor_grid, mask_grid
    else:        
        return vector, label_grid, sensor_grid, mask_grid, id_grid 
...
class DATA(Dataset):
    def __init__(self,logging, phase, sequence=1):
        data_path = 'VAEdata_CircleFOV40/'+phase +'/' 
        self.phase = phase

        if phase =='train':
            self.label_grid, self.sensor_grid, self.mask_grid = make_sequence(data_path, phase, sequence=sequence)
        else:
            self.vector, self.label_grid, self.sensor_grid, self.mask_grid, self.id_grid = make_sequence(data_path, phase, sequence=sequence)
    ...
    def __getitem__(self, index):
        ...
        mask_grid = self.mask_grid[index].copy()
        if self.phase != 'train':
...
def stack_tensors(label_grid, sensor_grid,mask_grid, id_grid=None, mask=None):
    label_grid = rearrange(label_grid)
    sensor_grid = rearrange(sensor_grid)
    mask_grid = rearrange(mask_grid)
    if id_grid is not None:
        id_grid = rearrange(id_grid)
    # if mask is not None:
    #     mask = rearrange(mask)
    return label_grid, sensor_grid,mask_grid, id_grid # ,  mask


# def reconstruction_loss(x, x_recon,x_mask):
#     """[summary]
...
#     cross_loss = cross_loss(x_recon*x_mask,x_mask*x)
    
#     return 0.5*recon_loss+0.5*cross_loss
...
# def VAE_loss(x, recon_x, x_mask,mu, logvar):
#     MSE = reconstruction_loss(x, recon_x,x_mask)
#     return MSE
...
def Label_vae_evaluate(beta, logging, loader, model, epoch=None):
    ...
    model.eval()
    with torch.no_grad():
        for vector, label_grid, sensor_grid,mask_grid, id_grid in loader:
            """
            vector : (T*N, 36)
            *_grid : (T*N, 120, 120)
            """    
            label_grid, sensor_grid, mask_grid, id_grid = stack_tensors(label_grid, sensor_grid, mask_grid, id_grid)  # (N,T, ...) --> (T*N, ...)       

            mu_l, logvar_l, z_l, decoded_l = model(label_grid,label_grid)     #这里还不知道怎么改
        ...
        if epoch == None:    
            loss = logging.info('(Test) recon_loss: {:.4f}'. format(avg_recon_loss))  
            save_path = out_dir+'/Label_VAE_test_sample/'
        else:
            loss = logging.info('(Eval Epoch {:d}) recon_loss: {:.4f}'. format(epoch, avg_recon_loss))   
...
def Label_vae_train(beta, logging, train_loader, validation_loader, model, ckpt_path, num_epochs, learning_rate = 0.001):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        if epoch % 20 == 0 :
            ...
            model.train()
        for label_grid, sensor_grid, mask_grid in loop: 

            label_grid, sensor_grid, mask_grid, _ = stack_tensors(label_grid, sensor_grid, mask_grid)  # (N,T, ...) --> (T*N, ...)       
            #print("********314 lines*******",mask_grid.shape)
            # num_ones = torch.sum(mask_grid == 1)
            # num_zeros = torch.sum(mask_grid == 0)
            # # 输出结果
            # print("1 的个数：", num_ones.item())
            # print("0 的个数：", num_zeros.item())

            mu_l, logvar_l, z_l, decoded_l = model(label_grid,label_grid)   #这里也还没有改
            #**********这里原来是label_grid********************
              
            #print("after decoder:",decoded_l.shape)
            #print("after decoder label:",label_grid.shape)
            # recon_loss = VAE_loss(sensor_grid, decoded_l, mask_grid,mu_l, logvar_l)   
            recon_loss, k_loss = VAE_loss(label_grid, decoded_l, mu_l, logvar_l)
            loss = recon_loss + k_loss * beta
        ...
        # logging.info('(Epoch {:d}) recon_loss: {:.4f}'.
        #              format(epoch, avg_recon_loss))      
...
if __name__ == "__main__":
    ...
    num_epochs = 3000 # 60 for the turtlebot experiment
    beta = 0.00025
    vae_learning_rate =  0.012 
    ...
    output_path = 'LabelCNN_CircleFOV424'
    # output_path = 'LabelCNN_CircleFOV40'

    # configure logging
    out_dir = 'data/'+ output_path  
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    log_file = os.path.join(out_dir, 'label_cnn_train.log')
    ...
    if encoder_type == 'vae':
        ...
        label_vae = nn.DataParallel(label_vae) # Use DataParallel to pac the model

        
    label_vae.to(device)
    # label_vae = DataParallel(label_vae, device_ids=[0,1,2,3]) # 额外加这一行多卡训练
    ...
    print("train_set:",np.array(train_set).shape)
