...
class CrowdSimDict(CrowdSim):
    ...
    def set_robot(self, robot):
        ...
        # d['rotated_robot_all_states'] = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(1, self.robot_max_save_states_length, 8), dtype = np.float32)
        # #正十二边形点集
    ...
    def generate_ob(self, reset):
        ob = {}
        ego_id = 100 # robot_id
        self.dict_update() #更新人群和机器人的状态字典
        ego_dict, other_dict = self.ego_other_dict(ego_id) #做成自己的字典和其他agent的字典（仅供机器人）
        ...
        if self.gridsensor == 'sensor' or self.collectingdata:
            visible_id, sensor_grid, ob['poly_occ_points'] = generateSensorGrid(label_grid, ego_dict, other_dict, map_xy, self.FOV_radius, res=self.grid_res)
            self.visible_ids.append(visible_id)
        else:
            visible_id = np.unique(label_grid[1])[:-1]
            self.visible_ids.append(visible_id)
        
        #可见id队列长度为human_num，全部填充999为不可见
        ob['visible_id'] = np.full((self.human_num), 999, dtype=np.float32)
        #如果可见id不为空，则传递的数组包含其id，替换999
        if len(visible_id) != 0:
            for num in visible_id:
                ob['visible_id'][num] = num


        #更新agent的自身历史状态列表
        self.update_agent_history_queue(visible_id)

        #所有状态分成两个矩阵输出到字典
        ob['rotated_humans_all_states'], ob['rotated_robot_all_states'] = self.rotated_humans_robot_all_states()
        
        #获得FOV点集合,已转换为机器人坐标系
        ob['FOV_points'] = self.get_FOV_points(0, 0, self.FOV_radius)

        #获得遮挡多边形点集
        # ob['cover_point'] = self.get_cover_points()
        ...
        if self.collectingdata:
            ...
            ob['vector'].extend(list(np.ravel(self.last_human_states))) # add human states to vector np.ravel：将多维数据展平为一维数组
            ob['grid_xy'] = map_xy
    ...
    #生成vectornet需要的状态观测
    # def generate_states_ob(self):
        
    #     human_vectors = np.concatenate((human.get_history_states_np() for human in self.humans), axis=0)
    #     robot_vectors = self.robot.get_history_states_np()
    #     #vector_obs的输出是humans和robots状态的堆叠
    #     return np.concatenate((human_vectors, robot_vectors), axis=0)

    #根据可见性更新每个agent的状态保存情况
    def update_agent_history_queue(self, visible_id_):
        # time = time.time() #时间选取由于多线程还有问题，以后直接增加time_step时间？
        #更新humans,可见即加入历史观测队列，不可见直接消除所有的历史观测（可以增加允许看不到的时间间隔）
        if len(visible_id_) == 0 :
            pass
        else:
            for human_id, human in enumerate(self.humans):
                if human_id in visible_id_:
                    human.save_visible_state(human_id, self.global_time)
                else:
                    human.clear_self_history_states()
        #更新robot
        self.robot.save_robot_state(self.global_time)

    #拼接所有humans状态做成字典，和机器人状态输出给后续的vector-net
    def rotated_humans_robot_all_states(self):
        #拼接所有humans相对状态矩阵
        humans_rotated_states = [self.transform_to_relative_coordinates(human.get_history_states_np(), self.humans_max_save_states_length) for human in self.humans]
        humans_states_np = np.concatenate(humans_rotated_states, axis=0)
        #获得机器人历史状态矩阵
        robot_states_np = self.transform_to_relative_coordinates(self.robot.get_history_states_np(), self.robot_max_save_states_length)
        #返回两个变换后的矩阵
        return humans_states_np, robot_states_np

    #获得FOV（十二边型）点集
    def get_FOV_points(self, robot_px, robot_py, FOV_radius):
        # 生成十二边形的顶点
        angles = np.linspace(0, 2 * np.pi, 12, endpoint=False)
        vertices_x = robot_px + FOV_radius * np.cos(angles)
        vertices_y = robot_py + FOV_radius * np.sin(angles)

        # 将顶点坐标组合成数组
        hexagon_vertices = np.column_stack((vertices_x, vertices_y))
    ...
    #将robot或human传回的单个状态矩阵转换到当前机器人坐标系下并且填充维度到 queque_max_length * feature_dim
    def transform_to_relative_coordinates(self, state_matrix, queque_max_length):

        def rotate_to_robot_state():
            id, px, py, vx, vy, theta, radius, timestamp = state_matrix[:, 0], state_matrix[:, 1], state_matrix[:, 2], state_matrix[:, 3] \
                    ,state_matrix[:, 4], state_matrix[:, 5], state_matrix[:, 6], state_matrix[:, 7]
            
            # 计算相对坐标
            relative_px = px - self.robot.px
            relative_py = py - self.robot.py

            # 计算相对速度
            relative_vx = vx - self.robot.vx
            relative_vy = vy - self.robot.vy
            # 计算相对偏向角
            relative_theta = theta - self.robot.theta

            relavite_time = timestamp - self.global_time
            #所有变换后的拼接在一起
            relative_attributes = np.column_stack((id, relative_px, relative_py, relative_vx, relative_vy, relative_theta, radius, relavite_time))

            return relative_attributes
        
        #如果是行人并且处于不可见状态，直接返回
        if state_matrix[0][0] == 999:
            return state_matrix
        
        #如果是机器人状态矩阵
        if state_matrix[0][0] == 100:
            #转换为当前坐标系
            rotated_states = rotate_to_robot_state()
            #填充缺失的维度
            for lack_num in range(queque_max_length - rotated_states.shape[0]):
                rotated_states = np.vstack([rotated_states, [100, 0, 0, 0, 0, 0, 0, -1]])
                
        #不是机器人也不是不可见的行人，则为可见行人的保存信息
        rotated_states = rotate_to_robot_state()
        #补充缺失维度信息
        for lack_num in range(queque_max_length - rotated_states.shape[0]):
            rotated_states = np.vstack([rotated_states, [rotated_states[0][0], 0, 0, 0, 0, 0, 0, -1]])
        #补充完整的矩阵
        assert rotated_states.shape == (queque_max_length, 8)
