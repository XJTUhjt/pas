...
class CrowdSimDict(CrowdSim):
    ...
    def generate_ob(self, reset):
        ...
        # #更新agent的自身历史状态列表
        self.update_agent_history_queue(self.visible_ids)
    ...
    def transform_to_relative_coordinates(self, state_matrix, queque_max_length):
        ...
        def rotate_to_robot_state():
            if state_matrix[0][0] == 100:
                id, px, py, gx, gy, vx, vy, theta, radius, timestamp = state_matrix[:, 0], state_matrix[:, 1], state_matrix[:, 2], state_matrix[:, 3] \
                        ,state_matrix[:, 4], state_matrix[:, 5], state_matrix[:, 6], state_matrix[:, 7], state_matrix[:, 8], state_matrix[:, 9]
            else:
            ...
            if state_matrix[0][0] == 100:
                relative_gx = gx - self.robot.px
                relative_gy = gy - self.robot.py
            ...
            #所有变换后的拼接在一起
            if state_matrix[0][0] == 100:
                relative_attributes = np.column_stack((id, relative_px, relative_py, relative_gx, relative_gy, relative_vx, relative_vy, relative_theta, radius, relavite_time))
            else:
        ...
        if state_matrix[0][0] == 100:
            ...
            for lack_num in range(queque_max_length - rotated_states.shape[0]):
                rotated_states = np.vstack([rotated_states, [100, 0, 0, 0, 0, 0, 0, 0, 0, -1]])
        else:        
